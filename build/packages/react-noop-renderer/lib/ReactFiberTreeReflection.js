...
function assertIsMounted(fiber) {
  !(isFiberMountedImpl(fiber) === MOUNTED) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Unable to find node on an unmounted component.') : _prodInvariant('153') : void 0;
}
...
function findCurrentFiberUsingSlowPath(fiber) {
  ...
  if (!alternate) {
    ...
    !(state !== UNMOUNTED) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Unable to find node on an unmounted component.') : _prodInvariant('153') : void 0;
    if (state === MOUNTING) {
      return null;
    }
    return fiber;
  ...
  while (true) {
    ...
    if (a['return'] !== b['return']) {
      ...
      if (!didFindChild) {
        ...
        !didFindChild ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Child was not found in either parent set. This indicates a bug related to the return pointer.') : _prodInvariant('185') : void 0;
      }
    }

    !(a.alternate === b) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Return fibers should always be each others\' alternates.') : _prodInvariant('186') : void 0;
  }
  // If the root is not a host container, we're in a disconnected tree. I.e.
  // unmounted.
  !(a.tag === HostRoot) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Unable to find node on an unmounted component.') : _prodInvariant('153') : void 0;
